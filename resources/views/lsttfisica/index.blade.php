...
<div style="margin-top: 20px; margin-bottom: 20px," class="d-flex justify-content-between ">
                <p style="margin-left: 50px" class="text-start">Mostrando {{ $fisicas->firstItem() }} a {{ $fisicas->lastItem() }} de {{ $fisicas->total() }} resultados</p>

                <div style="margin-right: 150px" class="pagination-container">
                    <ul class="pagination d-flex">
                        @if ($fisicas->onFirstPage())
                            <li class="page-item disabled"><span class="page-link">Anterior</span></li>
                        @else
                            <li class="page-item"><a class="page-link" href="{{ $fisicas->previousPageUrl() }}">Anterior</a></li>
                        @endif

                        @for ($i = 1; $i <= $fisicas->lastPage(); $i++)
                            <li class="page-item {{ $i == $fisicas->currentPage() ? 'active' : '' }}">
                                <a class="page-link" href="{{ $fisicas->url($i) }}">{{ $i }}</a>
                            </li>
                        @endfor

                        @if ($fisicas->hasMorePages())
                            <li class="page-item"><a class="page-link" href="{{ $fisicas->nextPageUrl() }}">Siguiente</a></li>
                        @else
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('lsttfisica.store') }}" method="POST">
                        @csrf
                        <div class="mb-3">
...
<!-- Modal para Editar -->
    @foreach($fisicas as $fisicas)
    <div class="modal fade" id="editarModal{{ $fisica->id }}" tabindex="-1" aria-labelledby="editarModal" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <form method="POST" action="{{ route('lsttfisica.update', $fisica->id) }}">
                        ...
                        @method('PUT')
                            <div class="mb-3">
                                ...
                                <select id="estado" class="form-select" id="estado" name="estado" value="{{ $fisica->estado }}">
                            ...
                            <div class="mb-3">
                                <label for="numero_documento" class="form-label">Número de Documento</label>
                                <input type="text" class="form-control" id="numero_documento" name="numero_documento" value="{{ $fisica->numero_documento }}">
                            </div>
                            <div class="mb-3">
                                <label for="nombres" class="form-label">Nombres</label>
                                <input type="text" class="form-control" id="nombres" name="nombres" value="{{ $fisica->nombres }}">
                            </div>
                            <div class="mb-3">
                                <label for="apellidos" class="form-label">Apellidos</label>
                                <input type="text" class="form-control" id="apellidos" name="apellidos" value="{{ $fisica->apellidos }}">
                            </div>
                            <div class="mb-3">
                                <label for="telefono" class="form-label">Teléfono</label>
                                <input type="text" class="form-control" id="telefono" name="telefono" value="{{ $fisica->telefono }}">
                            </div>
                            <div class="mb-3">
                                <label for="genero" class="form-label">Género</label>
